...
import { X, User, Users, Pencil, UserPlus, AlertTriangle, LogOut, Copy, MessageCircle, MoreVertical, Eye, UserMinus, Shield, ChevronRight, FileText } from 'lucide-react';
...
export default function RoomInfoSidebar({
    ...
    const fetchPreviewImages = useCallback(async (rid: string) => {
        ...
        try {
            ...
            const images = data.filter((m: MediaMessage) => m.type === 'image');
            const files = data.filter((m: MediaMessage) => m.type === 'file');
                    ...
                    {fetchingInfo ? (
                        ...
                        <>
                            ...
                            <div className="text-center">
                                ...
                                {/* Social Links — icon row, accent color, tooltip on hover */}
                                    ...
                                    <div className="flex items-center justify-center gap-3 mt-2">
                                        {privatePartner.social_links.map(link => (
                                            <a
                                                ...
                                                className="group relative opacity-60 hover:opacity-100 text-[var(--accent-color)] transition-opacity duration-150"
                                            >
                                                <PlatformIcon type={link.type} className="w-4 h-4" />
                                                {/* Tooltip */}
                                                <div className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 flex flex-col items-center opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                                                    <div className="px-2 py-1 rounded-md bg-[var(--bg-primary)] border border-[var(--border-color)] whitespace-nowrap">
                                                        <p className="text-[10px] font-semibold text-[var(--text-primary)] leading-tight">{getPlatformName(link.type)}</p>
                                                        <p className="text-[9px] text-[var(--text-muted)] leading-tight">@{getUsernameFromUrl(link.url, link.type)}</p>
                                                    </div>
                                                </div>
                                            </a>
                                        ))}
                                ...
                                <div className="w-full mt-4 bg-[var(--bg-primary)] p-4 rounded-xl border border-[var(--border-color)]">
                                    <div className="flex items-center justify-between mb-3">
                                        <div>
                                            ...
                                            <p className="text-[13px] font-semibold text-[var(--text-primary)]">Shared Media</p>
                                        </div>
                                        <button
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {sharedImages.slice(0, 5).map((item) => (
                                            <button
                                                key={item.id}
                                                onClick={() => setShowGallery(true)}
                                            ...
                                            >
                                                {item.type === 'image' ? (
                                                    ...
                                                    <div className="w-full h-full flex flex-col items-center justify-center p-2">
                                                        <FileText size={24} className="text-[var(--text-muted)] mb-1" />
                                            ...
                                            >
                                                {sharedImages[5].type === 'image' ? (
                                                    ...
                                                    <div className="w-full h-full flex flex-col items-center justify-center p-2">
                                                        <FileText size={24} className="text-[var(--text-muted)] mb-1" />
                                                        <span className="text-[10px] text-[var(--text-muted)] text-center truncate w-full">
                                                            {sharedImages[5].file_name || 'File'}
                                                        </span>
                                                    </div>
                                                )}
                                                {(totalImageCount + totalFileCount) > 6 && (
                                                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center rounded-md">
                                                        <span className="text-white text-[14px] font-bold">+{(totalImageCount + totalFileCount) - 5}</span>
                                                    </div>
                                                )}
                                            </button>
                                        )}
                    ...
                    {sharedImages.length > 0 && (
                        <div className="flex flex-col p-6 border-b border-[var(--border-color)] shrink-0">
                            <div className="flex items-center justify-between mb-4">
                                <div>
                                    <p className="text-[10px] font-bold text-[var(--accent-color)] tracking-widest uppercase mb-0.5">Gallery</p>
                                    <h3 className="text-[13px] font-semibold text-[var(--text-primary)]">Shared Media</h3>
                                </div>
                                <button
                                    onClick={() => setShowGallery(true)}
                                    ...
                                    >
                                        ...
                                        {(totalImageCount + totalFileCount) > 6 && (
                                            <div className="absolute inset-0 bg-black/60 flex items-center justify-center rounded-xl">
                                                <span className="text-white text-[15px] font-bold">+{(totalImageCount + totalFileCount) - 5}</span>
                                            </div>
                                        )}
                                    </button>
                                )}
