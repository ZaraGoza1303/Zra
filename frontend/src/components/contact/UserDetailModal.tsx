...
import { X, MessageSquare, UserPlus, Clock, UserCheck, Loader2, UserX, ShieldOff } from 'lucide-react';
...
const getIconColor = () => 'text-[var(--accent-color)]';
...
export default function UserDetailModal({
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                const [linksRes, blockedRes] = await Promise.all([
            ...
            <div className="relative w-full max-w-md bg-[var(--bg-secondary)] border border-[var(--border-light)] rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6">
                    ...
                    <div className="flex flex-col items-center mb-6">
                        ...
                        {/* Social Links — inline below username, no bg */}
                            ...
                            <div className="flex items-center justify-center gap-3 mt-2 mb-1">
                                {socialLinks.map((link) => (
                                    <a
                                        ...
                                        title={getPlatformName(link.type)}
                                        className={`transition-opacity duration-150 opacity-60 hover:opacity-100 ${getIconColor()}`}
                                    >
                        ...
                        {user.friendship_status && (
                            <span className={`mt-2 px-3 py-1 rounded-full text-xs font-medium ${user.friendship_status === 'friend' ? 'bg-emerald-500/15 text-emerald-400' :
                    ...
                    <div className="flex flex-col gap-3 mt-6">
                        ...
                        {isBlocked ? (
                            ...
                            <button
                                ...
                                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-medium text-red-400 hover:bg-red-500/10 border border-red-500/30 hover:border-red-500/60 disabled:opacity-50 transition-colors"
                            >
